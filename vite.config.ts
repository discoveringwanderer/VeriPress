...
function veriPressSharedDataApi(): Plugin {
  const dataDirectory = path.resolve(__dirname, '.veripress-data')
  ...
  function middleware(req: any, res: any, next: () => void) {
    const match = req.url?.match(/^\/api\/veripress\/([^?]+)$/)
    if (!match || !['GET', 'PUT'].includes(req.method)) return next()
    ...
    if (req.method === 'GET') {
      res.end(JSON.stringify({ value: store[key] ?? null }))
      return
    }
    ...
    req.on('end', () => {
      try {
        store[key] = JSON.parse(body).value
        writeStore(store)
        res.end(JSON.stringify({ ok: true }))
      } catch {
        res.statusCode = 400
        res.end(JSON.stringify({ error: 'Invalid shared data' }))
      }
    })
  }

  return {
    name: 'veripress-shared-data-api',
